...
EPOCH = 5
sequenceLength = 15
setup_name = "conv_16_8_lstm_4000"
...
def main(num_epochs=EPOCH):
    ...
    x = TimeDistributed(Conv2D(320, (3, 3), padding='same'), input_shape=(sequenceLength,40,40,1))(inputs)
    x = TimeDistributed(MaxPooling2D((2, 2)))(x)
    x = TimeDistributed(Conv2D(160, (3, 3), padding='same'))(x)
    x = TimeDistributed(MaxPooling2D((2, 2)))(x)
    x = TimeDistributed(Conv2D(160, (3, 3), padding='same'))(x)
    x = TimeDistributed(MaxPooling2D((2, 2)))(x)
    ...
    x = LSTM(4000, activation='tanh')(x)
    print(K.int_shape(x))

    x = RepeatVector(15)(x)
    print(K.int_shape(x))

    x = LSTM(4000, activation='tanh', return_sequences=True)(x)
    print(K.int_shape(x))

    # x = Reshape((15,10,10,4))(x)
    x = TimeDistributed(Reshape((5,5,160)))(x)
    
    x = TimeDistributed(Conv2D(160, (3, 3), padding='same'))(x)
    x = TimeDistributed(UpSampling2D((2, 2)))(x)
    x = TimeDistributed(Conv2D(160, (3, 3), padding='same'))(x)
    x = TimeDistributed(UpSampling2D((2, 2)))(x)
    x = TimeDistributed(Conv2D(320, (3, 3), padding='same'))(x)
    x = TimeDistributed(UpSampling2D((2, 2)))(x)
